...
void miniunz_help()
{
    printf("Usage : miniunz [-e] [-x] [-v] [-l] [-p password] file.zip [file_to_extr.] [-d extractdir]\n\n" \
...
int miniunz_extract_currentfile(unzFile uf, int opt_extract_without_path, const char *password)
{
    unz_file_info64 file_info = {0};
    FILE* fout = NULL;
    void* buf = NULL;
...
int miniunz_extract_all(unzFile uf, int opt_extract_without_path, const char *password)
{
    int err = unzGoToFirstFile(uf);
    if (err != UNZ_OK)
    {
    ...
    do
    {
        err = miniunz_extract_currentfile(uf, opt_extract_without_path, password);
        if (err != UNZ_OK)
            break;
        err = unzGoToNextFile(uf);
    }
...
int miniunz_extract_onefile(unzFile uf, const char *filename, int opt_extract_without_path, const char *password)
{
    ...
    if (miniunz_extract_currentfile(uf, opt_extract_without_path, password) == UNZ_OK)
        return 0;
    return 1;
}
    ...
    else if (opt_do_extract == 1)
    {
        ...
        else
        {
            if (filename_to_extract == NULL)
                ret = miniunz_extract_all(uf, opt_do_extract_withoutpath, password);
            else
                ret = miniunz_extract_onefile(uf, filename_to_extract, opt_do_extract_withoutpath, password);
        }
        if (opt_extractdir && CHDIR(current_dir))
        {
            printf("Error returning into %s, aborting\n", current_dir);
        }
    }
