...
void minizip_help()
{
    printf("Usage : minizip [-a] [-0 to -9] [-p password] [-j] file.zip [files_to_add]\n\n" \
...
int zip_run(int argc, char *argv[])
{
    ...
    if (check_file_exists(zipfilename) == 0)
        opt_overwrite = opt_overwrite == APPEND_STATUS_ADDINZIP ? 
            APPEND_STATUS_CREATEAFTER : APPEND_STATUS_CREATE;
